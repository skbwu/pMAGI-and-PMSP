...
class LorenzPINN(tf.keras.Model):
    ...
    def call(self, t, TT, TM, TFC, is_forecasting):
        ...
        if is_forecasting:
            t_physics = tf.convert_to_tensor(np.linspace(start=TM, stop=TM+TFC, 
                                                         num=math.ceil(TFC*40)+1).reshape(-1, 1))
    ...
    def get_loss(self, t, TT, TM, TFC, is_forecasting, u_true):
        return self.__mse(u_true, self(t, TT, TM, TFC, is_forecasting), is_forecasting)
    ...
    def __mse(self, u_true, y_pred, is_forecasting):

        # squared-loss on ONLY THE LAST OBSERVATION!
        if is_forecasting:
            
            # query only the last entry at the END OF THE IN-SAMPLE
            loss_x = tf.reduce_mean(tf.square(y_pred[0][-1] - u_true[0][-1]))
            loss_y = tf.reduce_mean(tf.square(y_pred[1][-1] - u_true[1][-1]))
            loss_z = tf.reduce_mean(tf.square(y_pred[2][-1] - u_true[2][-1]))
        
        # squared-loss on the ENTIRE reconstruction loss
        else:
